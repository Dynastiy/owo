  ...
  <div class="container my-4">
    ...
    <div class="">
      <div class="grid lg:grid-cols-4 md:grid-cols-2 grid-cols-1 gap-4">
        <div v-for="(item, idx) in items" :key="idx" class="bg-white p-4 rounded-sm">
          ...
          <img
            ...
            alt=""
            class="object-cover h-[150px] w-full object-center"
          />
          <div class="mt-1">
            <!-- <h4 class="text-sm leading-tight">{{ item.content }}</h4> -->
            <div v-html="truncateHtmlContent(item.content, 100)" class="text-sm">

            </div>
            <router-link :to="`/blog/${item.slug}`"><button class="brand-btn-md w-full brand-primary mt-2">Read More</button></router-link>
          </div>
        </div>
      </div>
